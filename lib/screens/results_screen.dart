...
class _ResultsScreenState extends State<ResultsScreen> {
  ...
  void _onLockedPhotoTap() {
    Navigator.push(context, MaterialPageRoute(builder: (_) => PaywallScreen(onUnlocked: () { setState(() { _isPro = PlanManager.isPro; }); })));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Theme.of(context).scaffoldBackgroundColor,
        title: Text('${widget.photos.length} Photos Mili', style: const TextStyle(color: Colors.white)),
      ...
      bottomNavigationBar: _isPro ? null : SafeArea(child: Padding(padding: const EdgeInsets.all(16), child: SizedBox(width: double.infinity, height: 52, child: ElevatedButton(onPressed: _onLockedPhotoTap, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF6C63FF), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))), child: const Text('Sabhi Photos Unlock Karo — ₹49', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: Colors.white)))))),
    );
  }
}